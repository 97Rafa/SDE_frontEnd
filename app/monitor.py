# import asyncio
# from app.models import RequestM
# from app.kafka_producer import send_message

# POLLING_INTERVAL = 1

# async def start_monitoring_task(SessionLocal):
#     last_seen_id = 0
#     while True:
#         db = SessionLocal()
#         try:
#             new_requests = db.query(RequestM).filter(RequestM.uid > last_seen_id).order_by(RequestM.uid).all()
#             for request in new_requests:
#                 print(f"🔔 New Request added: {request.toJson()}")
#                  # ✅ Send to Kafka topic
#                 # send_message("request_topic", request.toJson())
#                 last_seen_id = request.uid
#         finally:
#             db.close()
